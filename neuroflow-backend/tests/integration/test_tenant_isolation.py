...
@pytest.fixture
async def two_orgs(db_session: AsyncSession) -> tuple[Actor, Actor, UUID]:
    ...
    return owner_a, owner_b, project_b.id


CrossTenantCase = Callable[[UUID, UUID], tuple[str, str, dict[str, Any] | None]]
...
async def test_cross_tenant_access_is_404_not_403(
    client: AsyncClient,
    two_orgs: tuple[Actor, Actor, UUID],
    case: CrossTenantCase,
) -> None:
    owner_a, owner_b, project_b_id = two_orgs
    method, path, body = case(owner_b.organization.id, project_b_id)
...
async def test_cross_tenant_member_removal_is_404(
    client: AsyncClient, two_orgs: tuple[Actor, Actor, UUID], db_session: AsyncSession
) -> None:
    owner_a, owner_b, _project_b_id = two_orgs
...
async def test_project_list_only_shows_the_caller_s_own_organization(
    ...
    owner_a, _owner_b, _project_b_id = two_orgs
