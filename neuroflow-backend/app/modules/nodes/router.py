...
async def list_node_types(controller: NodeTypeControllerDep) -> list[NodeTypeDescriptor]:
    return await controller.list_all()


@router.get("/{key}", response_model=NodeTypeDescriptor)
async def get_node_type(key: str, controller: NodeTypeControllerDep) -> NodeTypeDescriptor:
